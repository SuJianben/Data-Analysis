...
(function (window, document) {
  ...
  if (window.__tkfSignalUserIdentityV1) return;
  window.__tkfSignalUserIdentityV1 = true;

  var VISITOR_STORAGE_KEY = "tkf_signal_visitor_id";
  var SESSION_STORAGE_KEY = "tkf_signal_session_id";
  var config = Object.assign({ source: "shopify" }, window.TKF_SIGNAL_IDENTITY_CONFIG || {});
  ...
  function send(event) {
    ...
    var body = JSON.stringify({ source: config.source, event: event });
  ...
  loadPrivacyApi();
  window.TKFSignalIdentity = { track: track, refreshConsent: updatePermission };
})(window, document);
