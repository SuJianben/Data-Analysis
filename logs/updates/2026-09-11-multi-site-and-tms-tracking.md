...
- 新主题已发布到 Vercel 正式环境；线上 TKF/TMS 健康接口、TKF 全局点击接口和 TMS 用户接口均返回 `HTTP 200` 且 `ok=true`，数据健康页与 TMS 用户页真实加载通过。
- 首次远端构建因临时参考目录被 Vercel 一并上传而失败；删除仅用于对照的临时解压副本后重新构建成功，用户提供的原始 ZIP 未改动。
