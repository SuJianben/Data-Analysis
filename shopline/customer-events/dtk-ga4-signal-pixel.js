// DTK GA4 ecommerce + Signal - SHOPLINE Customer Events
const GA4_MEASUREMENT_ID = "G-2YN8WS6N3E";
const DTK_SIGNAL_EVENT_ENDPOINT = "https://multi-site-analytics.vercel.app/api/events";
...
function shouldFilterPageView(event) {
  ...
  return AUTOMATED_USER_AGENT.test(userAgent) || !event.clientId;
}
...
async function sendSignal(event) {
  ...
  const payload = { siteKey: "dtk", source: "shopline_pixel:dtk", event };
  let lastStatus = 0;
  let lastCode = "network_error";
  for (let attempt = 0; attempt < delays.length; attempt += 1) {
    if (delays[attempt] > 0) await wait(delays[attempt]);
    try {
      const response = await fetch(DTK_SIGNAL_EVENT_ENDPOINT, {
        method: "POST",
...
analytics.subscribe("dtk_signal_click", (event) => {
  const data = event.data || {};
  const clickParams = {
