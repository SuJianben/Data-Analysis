...
该子项目负责按站点保存 TKF、TMS、FKK、BLK 的 GA4 汇总报表与脱敏用户行为。D1 不保存姓名、邮箱、电话，也不保存 Shopify 或 SHOPLINE 的原始客户 ID。
...
四个 Worker 绑定同一个 D1，数据仍由载荷中的 `siteKey` 隔离。健康检查会返回与域名一致的服务名，方便从日志和监控中辨认站点。单独发布时可使用 `npm run deploy:tkf`、`npm run deploy:tms`、`npm run deploy:fkk` 或 `npm run deploy:blk`，禁止复制多套 Worker 源码分别维护。
...
SHOPLINE 客户事件同样运行在隔离环境中。BLK 的不透明来源入口只接受 `siteKey=blk`、`source=shopline_pixel:blk`，以及带 SHOPLINE 专属事件 ID、匿名 `clientId`、有效时间和对应字段的页面浏览、点击、加购、结账或购买事件；购买还必须携带合规的订单哈希、金额、币种和商品数量。这样能用同一个匿名访客标识串起转化路径，同时拒绝其他站点和格式不符的数据。
