...
import { getLocalDataHealthReport } from "@/features/health/local-health-report";
...
function localDashboardOverview(options: DateRangeOptions): DashboardOverview {
  return {
...
export async function loadDashboardOverview(options: DateRangeOptions = {}): Promise<DashboardOverview> {
  return readWithLocalFallback({
    label: "站点概览",
    local: () => localDashboardOverview(options),
    cloudflare: async () => {
...
export async function loadMenuReportRows(options: DateRangeOptions = {}): Promise<MenuReportRow[]> {
  return readWithLocalFallback({
    label: "菜单报表",
    local: () => getMenuReportRows(options),
    cloudflare: async () => {
...
function localGlobalClickReport(options: GlobalClickReportQuery): GlobalClickReport {
...
export async function loadGlobalClickReport(options: GlobalClickReportQuery = {}): Promise<GlobalClickReport> {
  return readWithLocalFallback({
    label: "全局点击报表",
    local: () => localGlobalClickReport(options),
    cloudflare: async () => {
      const query = new URLSearchParams();
...
export async function loadGlobalClickSummary(options: DateRangeOptions = {}): Promise<GlobalClickSummary> {
  return readWithLocalFallback({
    label: "全局点击汇总",
    local: () => localGlobalClickSummary(getGlobalClickReportRows(options)),
    cloudflare: async () => {
...
export async function loadGlobalClickTrend(options: DateRangeOptions = {}): Promise<GlobalClickTrendPoint[]> {
  return readWithLocalFallback({
    label: "全局点击趋势",
    local: () => getGlobalClickTrend(options),
    cloudflare: async () => {
...
export async function loadLatestAnalysis(): Promise<AnalysisResult | null> {
  if (appConfig.analyticsReadMode !== "local" && appConfig.userEventApiUrl) return null;
...
export async function loadDataHealthReport(site: SiteKey): Promise<DataHealthReport> {
  return readWithLocalFallback({
    label: "数据健康",
    local: () => getLocalDataHealthReport(site),
    cloudflare: async () => {
