...
export function UserDistributionScatter({ rows }: { rows: UserSummaryRow[] }) {
  return <DistributionScatterPanel title="用户分布气泡" description="按真实事件活跃度与访问页面覆盖范围查看用户分布；相同位置自动聚合。" xLabel="用户事件数（活跃度）" yLabel="访问页面数（覆盖范围）" quadrantLabels={{ "high-high": "高活跃·高覆盖", "low-high": "低活跃·高覆盖", "high-low": "高活跃·低覆盖", "low-low": "低活跃·低覆盖" }} pointUnit="一位用户" mode="bubble-density" bubbleItemLabel="用户" points={rows.map((row) => ({
    id: row.identityKey,
