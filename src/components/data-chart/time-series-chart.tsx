...
export function TimeSeriesChart({
  ...
  const points = datesInRange(dateRange).map((date) => ({
    date,
    values: Object.fromEntries(series.map(({ key }) => [key, Number(byDate.get(date)?.values[key] || 0)])),
  }));
  const visibleSeries = series.filter(({ key }) => points.some((point) => point.values[key] > 0));
  ...
  const max = Math.max(...points.flatMap((point) => visibleSeries.map(({ key }) => point.values[key])), 1);
  const activePoint = activeIndex === null ? null : points[activeIndex];
  const activeX = activeIndex === null ? 0 : isShortRange
    ? padding + activeIndex * groupWidth + groupWidth / 2
    : padding + (activeIndex / Math.max(points.length - 1, 1)) * (width - padding * 2);
  ...
  if (!data.length || !visibleSeries.length) return <div className="chart-empty">当前范围暂无趋势数据。</div>;

  return (
    <div className="chart-wrap">
      <svg className="dashboard-chart traffic-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={ariaLabel}>
        ...
        {isShortRange ? points.map((point, index) => {
          ...
          return (
            <g key={point.date} onMouseEnter={() => setActiveIndex(index)} onMouseLeave={() => setActiveIndex(null)}>
              <title>{`${formatDate(point.date)}：${visibleSeries.map(({ key, label }) => `${label} ${formatNumber(point.values[key])}`).join("，")}`}</title>
          ...
          <>
            ...
            {points.map((point, index) => {
              ...
              return <g key={point.date} className="traffic-point" onMouseEnter={() => setActiveIndex(index)} onMouseLeave={() => setActiveIndex(null)}>
                <title>{`${formatDate(point.date)}：${visibleSeries.map(({ key, label }) => `${label} ${formatNumber(point.values[key])}`).join("，")}`}</title>
                <circle cx={x} cy={height - padding - (Math.max(...visibleSeries.map(({ key }) => point.values[key])) / max) * chartHeight} r="9" className="traffic-point-hit" />
                {activeIndex === index && visibleSeries.map(({ key, color }) => <circle key={key} cx={x} cy={height - padding - (point.values[key] / max) * chartHeight} r="4" fill={color} stroke="white" strokeWidth="2" />)}
              </g>;
            })}
          </>
        )}
      ...
      {activePoint && (
        <div className="chart-tooltip" style={{ left: `${(activeX / width) * 100}%`, top: "18%" }} role="status">
          <span>{formatDate(activePoint.date)}</span>
          {visibleSeries.map(({ key, label }) => <strong key={key}>{label} {formatNumber(activePoint.values[key])}</strong>)}
        </div>
      )}
      {isShortRange ? (
        <div className="chart-axis traffic-axis" style={{ gridTemplateColumns: `repeat(${Math.max(points.length, 1)}, minmax(0, 1fr))` }}>
          {points.map((point) => <span key={point.date}>{formatDate(point.date)}</span>)}
        </div>
      ) : (
