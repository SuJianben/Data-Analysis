...
export function DistributionScatterPanel({ title, description, points, xLabel, yLabel, quadrantLabels, pointUnit = "一项数据", mode = "ranked-points", bubbleItemLabel = "数据项" }: { title: string; description: string; points: DistributionPoint[]; xLabel: string; yLabel: string; quadrantLabels: Record<DistributionQuadrant, string>; pointUnit?: string; mode?: "ranked-points" | "bubble-density"; bubbleItemLabel?: string }) {
  const note = mode === "bubble-density" ? `气泡大小代表同位置${bubbleItemLabel}数量 · 悬停查看明细` : `虚线为排名中位线 · 每个点代表${pointUnit}`;
  return <section className="user-distribution-panel" aria-label={title}><div className="section-heading"><div><span className="eyebrow">DISTRIBUTION</span><h2>{title}</h2><p>{description}</p></div><span className="chart-note">{note}</span></div>{mode === "bubble-density" ? <DistributionBubbleScatter points={points} xLabel={xLabel} yLabel={yLabel} quadrantLabels={quadrantLabels} itemLabel={bubbleItemLabel} ariaLabel={`${title}聚合气泡散点图`} /> : <DistributionScatter points={points} xLabel={xLabel} yLabel={yLabel} quadrantLabels={quadrantLabels} ariaLabel={`${title}散点图`} />}</section>;
}
