...
export default async function UserDetailPage({ params }: { params: Promise<{ visitorId: string }> }) {
  ...
  const rows = await loadUserEvents(identity.key);
  return (
    <div className="page page-enter">
      <header className="page-heading compact-heading">
        <div><Link className="back-link" href="/users">← 返回用户列表</Link><span className="section-number">04 / USER JOURNEY</span><h1>{userIdentityLabel(identity.type, identity.id)}</h1><p>按时间查看该用户的页面和交互轨迹。</p></div>
      </header>
