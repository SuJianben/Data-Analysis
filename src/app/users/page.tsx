...
import { loadUserSummaries, loadUserTrend } from "@/services/connectors/user-events";
...
export default async function UsersPage({ searchParams }: { searchParams: Promise<DateRangeParams> }) {
  ...
  const [rows, trend] = await Promise.all([loadUserSummaries(500, range), loadUserTrend(range)]);
